...
const { Vehicle, Route, Steersman, Profile, Trip } = require("../models/index");
...
exports.getReport = async (req, res) => {
  try {
    ...
    for (let i = 0; i < enterprises.length; i++) {
      let sumTi = 0;
      let sumMo = 0;
      var routes = await Route.find({ idEnterprise: enterprises[i]._id });
      for (let j = 0; j < routes.length; j++) {
        var trips = await Trip.find({ idRoute: routes[j]._id });
        for (let k = 0; k < trips.length; k++) {
          //var tickets = await Ticket.find({ idTrip: trips[k]._id });
          ...
          for (let h = 0; h < tickets.length; h++) {
            var ticket = tickets[h];
            for (const qua of ticket.quantity) {
              //console.log(qua);
              if (qua == true) {
                sumTi += 1;
              }
              sumMo += sumTi * tickets[h].price;
            }
          }
        }
      }
      list.push({ name: enterprises[i].name, sumTi: sumTi, sumMo: sumMo });
    }
