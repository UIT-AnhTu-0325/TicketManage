...
export const MyTicket = (props) => {
    ...
    const active = books.filter(item => new Date(item.trip.startDate) >= Date.now() && item.book.idUser === localStorage.getItem('id') && !item.book.canceled);
    const used = books.filter(item => new Date(item.trip.startDate) < Date.now() && item.book.idUser === localStorage.getItem('id') && !item.book.canceled);
    const canceled = books.filter(item => item.book.canceled && item.book.idUser === localStorage.getItem('id'));
